...
from .services import create_loan, list_loans, return_loan
...
def criar_emprestimo():
    ...
    db_path = current_app.config["DB_PATH"]
    try:
        loan = create_loan(db_path, payload)
...
def devolver_emprestimo():
    ...
    if loan_id is None or str(loan_id).strip() == "":
        return json_response(
            success=False, message="Campo obrigatorio ausente ou vazio: emprestimo_id.", status=400
        )

    db_path = current_app.config["DB_PATH"]
    try:
        loan = return_loan(db_path, str(loan_id).strip())
    except ValueError as exc:
        message = str(exc)
